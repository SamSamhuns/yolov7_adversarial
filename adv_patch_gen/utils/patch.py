...
class PatchTransformer(nn.Module):
    ...
    def forward(
        ...
        )  # [bsize, max_labels, pchannel, pheight, pwidth]
        ...
        if do_transforms:
            ...
            contrast = (
                ...
                .uniform_(self.min_contrast, self.max_contrast)
                .to(self.dev)
            )
            contrast = contrast.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            contrast = contrast.expand(
                -1, -1, adv_batch.size(-3), adv_batch.size(-2), adv_batch.size(-1)
            )
            ...
            brightness = (
                torch.FloatTensor(batch_size)
                .uniform_(self.min_brightness, self.max_brightness)
                .to(self.dev)
            )
            brightness = brightness.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            brightness = brightness.expand(
                -1, -1, adv_batch.size(-3), adv_batch.size(-2), adv_batch.size(-1)
            )
            ...
            noise = (
                torch.FloatTensor(adv_batch.size()).uniform_(-1, 1).to(self.dev)
                * self.noise_factor
            )
        ...
        cls_ids = lab_batch[..., 0].unsqueeze(
            -1
        )  # equiv to torch.narrow(lab_batch, 2, 0, 1)
        cls_mask = cls_ids.expand(-1, -1, 3)
        cls_mask = cls_mask.unsqueeze(-1)
        cls_mask = cls_mask.expand(-1, -1, -1, adv_batch.size(3))
        cls_mask = cls_mask.unsqueeze(-1)
        cls_mask = cls_mask.expand(
            -1, -1, -1, -1, adv_batch.size(4)
        )  # [bsize, max_preds, pchannel, pheight, pwidth]
        ...
        if do_rotate:
            angle = (
                torch.FloatTensor(anglesize)
                .uniform_(self.minangle, self.maxangle)
                .to(self.dev)
            )
        else:
        ...
        target_size = torch.sqrt(
            ((lab_batch_scaled[:, :, 3].mul(self.target_size_frac)) ** 2)
    ...
    def forward(self, img_batch, adv_batch):
        ...
        for adv in advs:
            ...
            # alpha blend
                ...
                alpha_blend = (
                    self.patch_alpha * adv + (1.0 - self.patch_alpha) * img_batch
                )
